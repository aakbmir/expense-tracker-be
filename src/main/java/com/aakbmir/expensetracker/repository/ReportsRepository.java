...
public interface ReportsRepository extends JpaRepository<Expense, Long> {
    ...
    @Query("SELECT i FROM Expense i WHERE YEAR(i.date) = :year AND MONTH(i.date) = :month and i.subCategory=:subCategory order by date desc")
    List<Expense> findSubCategoryByMonthAndYear(int year, int month, String subCategory);
